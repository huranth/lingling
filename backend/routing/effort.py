...
publish their own set of values they honour:
    ...
    deepseek-v4-flash-free      low  high  max    (+ an on/off toggle)
...
def clamp(effort: str, allowed: Optional[Sequence[str]]) -> Optional[str]:
    ...
    if want is None:
        return None
    # Round the distance before comparing: raw float drift makes an exact
    # midpoint look one-sided (0.5-0.3 == 0.2 but 0.7-0.5 == 0.19999999999999996),
    # so `medium` against [low, high] resolved to `high` -- spending more thinking
    # than the client asked for, contradicting the weaker-wins tie rule below.
